...
class BindingPressRelease extends Binding{
	...
	public override function getActions():Array<FlxActionDigital>{
		return [getMainAction(), pressedAction, releasedAction];
	}
	...
	public function getPressedAction():FlxActionDigital{
		return pressedAction;
	}

	public function getReleasedAction():FlxActionDigital{
		return releasedAction;
	}

	public function bindPressedActionKeys(keys:Array<FlxKey>):Void{
		for(key in keys)
			pressedAction.addKey(key, FlxInputState.JUST_PRESSED);
	}

	public function bindReleasedActionKeys(keys:Array<FlxKey>):Void{
		for(key in keys)
			releasedAction.addKey(key, FlxInputState.JUST_RELEASED);
	}
